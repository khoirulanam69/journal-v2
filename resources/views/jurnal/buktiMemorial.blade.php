...
<div class="jurnal-detail">
    ...
    <table class="table table-bordered" style="border-color: #000;" width="100%">
        ...
        <tbody>
            ...
            <tr>
                <td class="p-0 text-center" style="width:10%;">{{$detailjurnal['kd_rek']}}</td>
                <td class="p-0 text-center">{{$detailjurnal['nm_rek']}}</td>
                @if($detailjurnal['debet']>0)
                <td class="text-end p-0" style="width:10%;">{{number_format($detailjurnal['debet'], 2, ",", ".");}}</td>
                @else
                <td class="text-end p-0" style="width:10%;"></td>
                @endif
                @if($detailjurnal['kredit']>0)
                <td class="text-end p-0" style="width:10%;">{{number_format($detailjurnal['kredit'], 2, ",", ".");}}</td>
                @else
                <td class="text-end p-0" style="width:10%;"></td>
                @endif
            </tr>
